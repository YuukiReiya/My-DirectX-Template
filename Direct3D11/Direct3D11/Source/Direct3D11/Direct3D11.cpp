...
//	UINT createDeviceFlags = 0;
	...
	hr = m_pDevice->CreateRenderTargetView(
		...
		&m_pRenderTargetView			/*!< レンダーターゲットビューを受け取る変数 */
	);
	...
	hr = m_pDevice->CreateTexture2D(
		...
		&m_pDepthStencil	/*!< 作成したテクスチャを受けとる変数 */
	);
	if (FAILED(hr)) {
	...
	m_pDeviceContext->OMSetRenderTargets(
		1,							/*!< 描画ターゲット数 */
		&m_pRenderTargetView,		/*!< 描画ターゲットビュー */
		m_pDepthStencilView			/*!< 深度 / ステンシルビュー */
	);
	...
	hr = m_pDevice->CreateDepthStencilState(
		...
		&m_pDepthStencilState		/*!< 作成した 深度 /ステンシル・ステートを受け取る変数 */
	);
	if (FAILED(hr)) { 
	...
	m_pDeviceContext->OMSetDepthStencilState(
		m_pDepthStencilState,		/*!< 深度 /ステンシル・ステート */
		0							/*!< ステンシル・テストで参照値 */
	);
...
void Direct3D11::Clear()
{
	...
	m_pDeviceContext->ClearRenderTargetView(
		m_pRenderTargetView,	/*!< クリアするレンダーターゲットビュー */
		c_ClearColor			/*!< クリアカラー */
	);

	/*! デプス・ステンシルビューのクリア */
	m_pDeviceContext->ClearDepthStencilView(
		m_pDepthStencilView,	/*!< クリアするデプス・ステンシルビュー */
